  ...
  onFileChange(event: any) {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        const totalScoresSheet = workbook.SheetNames.find(name => 
          name.toLowerCase().includes('total') && name.toLowerCase().includes('scores')
        );
        
        if (!totalScoresSheet) {
          console.log('Available sheets:', workbook.SheetNames.join(', '));
          throw new Error('Could not find "Total Scores" sheet. Available sheets: ' + workbook.SheetNames.join(', '));
        }

        const worksheet = workbook.Sheets[totalScoresSheet];
